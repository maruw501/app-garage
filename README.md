...
APP GARAGE は、AIアプリクリエイターとして、無料で使えるWebアプリの公開と、企業・店舗・介護施設・クリニック・個人事業主向けの専用アプリ開発・ホームページ作成相談を受け付ける静的ホームページです。

APP GARAGEの紹介サイトはHTML、CSS、JavaScriptだけで動作し、Cloudflare Pagesの無料プランへそのまま公開できます。個別の無料アプリでは、必要な機能に応じて外部のデータ保存・認証サービスやGoogle AdSenseを使用する場合があります。
...
返品保証: 納品後2週間以内の連絡でアプリを回収し、開発費をいただかない
```

料金欄には、企画・設計・制作のすべての工程でAIを活用し、開発時間を短縮することで価格を抑えていることを記載しています。

## 受注範囲の表示

受注範囲は `index.html` の `service` セクションと `terms.html` で案内しています。

基本的にAIを活用して安全に開発・動作確認できる規模と内容を受け付けます。高度なセキュリティ要件、大規模な外部システム連携、専門的な法令対応などが必要な案件は、内容確認後にお断りする場合があります。
